...
## The company
**Magmos — real-time USDC payroll & remittances on Arc.**
...
1. **Title** — "Magmos" wordmark, magma-glow. Subtitle "Payroll that arrives every second." Chips: *Built on Arc · Powered by Circle USDC · Track 1: UAE → Global remittances.*
...
8. **Live proof / demo** — "Not a mockup — **6 real on-chain transactions** on Arc testnet." Show a row of proof chips (Pool funded ✓, Stream started ✓, Invoice paid ✓, Treasury→yield ✓, Worker claimed ✓, Bridged home ✓) each with a truncated tx hash and a green ✓. Mention a 5-minute narrated demo video exists.
9. **Why Arc + Circle** — The unfair advantage: **USDC is the native gas token** (no volatile gas, sub-cent fees), **CCTP v2** for native cross-chain USDC, **Circle Wallets/passkeys** for onboarding, **instant finality**. Contrast a small "traditional rails vs Magmos on Arc" comparison (fee %, settlement time, transparency).
